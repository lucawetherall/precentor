...
export default async function RotaPage({ params }: Props) {
  ...
  try {
    // The service list and the membership list are independent, so fetch them
    // in one parallel wave rather than back-to-back.
    ...
    if (upcomingServices.length > 0) {
      ...
      // None of these four reads depend on each other (only on serviceIds /
      // churchId), so run them as a single parallel wave.
