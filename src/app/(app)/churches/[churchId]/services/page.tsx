...
export default async function ServicesPage({ params }: Props) {
  ...
  try {
    ...
    // Availability, music slots, and rotas all depend on serviceIds but not on each other — fetch in parallel.
    ...
    type RotaRows = Awaited<typeof rotasQuery>;

    const [userAvailability, slots, rotas]: [AvailabilityRows, SlotRows, RotaRows] =
      serviceIds.length > 0
        ? await Promise.all([availabilityQuery, slotsQuery, rotasQuery])
        : [[], [], []];
